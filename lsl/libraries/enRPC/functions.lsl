...
integer _ENRPC_DIALOG_LSN; // used by enCLEP_DialogListen()
string _ENRPC_SOURCE_REGION; // used by FEATURE_ENCLEP_STAGE_SOURCE_REGION
string _ENRPC_SOURCE_PRIM; // used by FEATURE_ENCLEP_STAGE_SOURCE_PRIM
...
/*!
An external loader for HTTP parameters is needed to simplify _enCLEP_Send(), since lists don't work with the preprocessor.
*/
enCLEP_StageHTTPParameters(
...
//  internal function that runs llListen on everything in _ENRPC_CLEP - DON'T run this without running _enCLEP_UnListenAll() first!
_enCLEP_ListenAll()
{
...
)
{
    ...
    #if defined FEATURE_ENCLEP_PROTOCOL_SIGNING
        // signing is enabled - are we signing this message?
        string private_key = enCLEP_GetKey(key_name); // import private_key from ENRPC_KEYS
    ...
    #if (defined FEATURE_ENCLEP_PROTOCOL_LEP || defined FEATURE_ENCLEP_PROTOCOL_LEP_OUTBOUND)
        if (flags & FLAG_ENRPC_PROTOCOL_LEP)
        {
            llMessageLinked(
                target_link,
    ...
    #else
        if (flags & FLAG_ENRPC_PROTOCOL_LEP)
            enLog_Error("FEATURE_ENCLEP_PROTOCOL_LEP[_OUTBOUND] not defined");
    #endif

    #if (defined FEATURE_ENCLEP_PROTOCOL_CLEP || defined FEATURE_ENCLEP_PROTOCOL_CLEP_OUTBOUND)
    ...
    #else
        if (flags & FLAG_ENRPC_PROTOCOL_CLEP)
            enLog_Error("FEATURE_ENCLEP_PROTOCOL_CLEP[_OUTBOUND] not defined");
    #endif
    #if (defined FEATURE_ENCLEP_PROTOCOL_CLEP || defined FEATURE_ENCLEP_PROTOCOL_CLEP_OUTBOUND) && defined FEATURE_ENCLEP_PROTOCOL_CLEP_SHOUT
            else llShout(channel, llDumpList2String(data, "\n")); // shout if prim is not in region and FEATURE_ENCLEP_PROTOCOL_CLEP_SHOUT is defined
    #elif (defined FEATURE_ENCLEP_PROTOCOL_CLEP || defined FEATURE_ENCLEP_PROTOCOL_CLEP_OUTBOUND) && defined FEATURE_ENCLEP_PROTOCOL_CLEP_SAY
            else llSay(channel, llDumpList2String(data, "\n")); // say if prim is not in region and FEATURE_ENCLEP_PROTOCOL_CLEP_SAY is defined
    #elif (defined FEATURE_ENCLEP_PROTOCOL_CLEP || defined FEATURE_ENCLEP_PROTOCOL_CLEP_OUTBOUND) && defined FEATURE_ENCLEP_PROTOCOL_CLEP_WHISPER
            else llWhisper(channel, llDumpList2String(data, "\n")); // whisper if prim is not in region and FEATURE_ENCLEP_PROTOCOL_CLEP_WHISPER is defined
    #endif
    #if (defined FEATURE_ENCLEP_PROTOCOL_CLEP || defined FEATURE_ENCLEP_PROTOCOL_CLEP_OUTBOUND)
        }
    #endif

    #if (defined FEATURE_ENCLEP_PROTOCOL_SNEP || defined FEATURE_ENCLEP_PROTOCOL_SNEP_OUTBOUND)
    ...
    #else
        if (flags & FLAG_ENRPC_PROTOCOL_SNEP)
            enLog_Error("FEATURE_ENCLEP_PROTOCOL_SNEP[_OUTBOUND] not defined");
    #endif
...
)
{
    ...
    #if !defined FEATURE_ENCLEP_ALLOW_ALL_LEP_DOMAINS
    ...
    #if defined FEATURE_ENCLEP_SIGNATURE_VERIFICATION
