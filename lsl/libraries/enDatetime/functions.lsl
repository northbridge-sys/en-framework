...
*/
integer enDatetime_AddMillisec(
    integer ms,
    integer add
)
{
...
*/
integer enDatetime_DaysInMonth(
    integer y,
    integer m
)
{
...
@return integer Number of days in specified month.
*/
integer enDatetime_DaysInYM(
    list ym
)
{
    return enDatetime_DaysInMonth(llList2Integer(ym, 0), llList2Integer(ym, 1));
}
...
*/
float enDatetime_EnvironmentToDaypart(
    vector p
)
{
...
*/
string enDatetime_HMToPretty(
    list hm,
    integer flags
)
{
    hm = _enDatetime_HTo12(hm, flags);
    hm = _enDatetime_PadZeroesHMS(hm, flags);
...
DOES NOT support FLAG_ENDATETIME_12_HOUR.
...
@return string Pretty string.
*/
string enDatetime_HMSUToPretty(
    list hmsu,
    integer flags
)
{
...
/*!
Function wrapper for _enDatetime_MToPretty() macro.
Can be called in multiple places with minimal memory impact.
This is used for optimization. If you only call enDatetime_MToPretty() once in the script, it's slightly more efficient to use the _enDatetime_MToPretty() macro directly.
@param integer month Month of year (1-12).
@return string Full textual representation of a month.
*/
string enDatetime_MToPretty(
    integer month
)
{
    return _enDatetime_MToPretty(month);
}

/*!
Converts a timestamp into an enDatetime millisec, an integer that can store a month's range of milliseconds.
enDatetime millisecs are not immediately related or aligned to any specific timeframe. They may only be compared with each other and modified using enDatetime functions.
@param string timestamp ISO 8601 timestamp from llGetTimestamp().
@return integer enDatetime millisec.
*/
integer enDatetime_TimestampToMillisec(
    string timestamp // llGetTimestamp string (use enDatetime_NowToMillisec() for the current value)
    )
{
...
Note that the enDatetime list format does not require µs (microseconds); this is the only function that returns it.
...
Resultant enDatetime list DOES HAVE subsecond precision; other methods may not have subsecond precision. Round s if needed.
@param integer t ISO 8601 timestamp. See llGetTimestamp().
@return list [Y, M, D, h, m, s, u].
*/
list enDatetime_TimestampToYMDHMSU(
    string t
)
{
...
*/
string enDatetime_UnixToDOW(
    integer u
)
{
    return llList2String(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"], enDatetime_UnixToIOW(u));
}
...
*/
integer enDatetime_YMDHMSDiffToSeconds(
    list ymdhms_a,
    list ymdhms_b
)
{
    return enDatetime_YMDHMSToUnix(ymdhms_a) - enDatetime_YMDHMSToUnix(ymdhms_b);
}
...
*/
string enDatetime_YMDHMSToPretty(
    list ymdhms,
    integer flags
)
{
...
*/
integer enDatetime_YMDHMSToUnix(
    list ymdhms
)
{
...
/*!
Converts YMDHMSU list to enDatetime millisec.
@param list ymdhmsu [Y, M, D, h, m, s, u].
@return integer enDatetime millisec.
*/
integer enDatetime_YMDHMSUToMillisec(
    list ymdhmsu
)
{
    return enDatetime_TimestampToMillisec(enDatetime_YMDHMSUToTimestamp(ymdhmsu));
}
...
{
    ymdhmsu = _enDatetime_PadZeroesIndex(ymdhmsu, 1, 2); // pad MM
    ymdhmsu = _enDatetime_PadZeroesIndex(ymdhmsu, 2, 2); // pad DD
    ymdhmsu = _enDatetime_PadZeroesIndex(ymdhmsu, 3, 2); // pad hh
    ymdhmsu = _enDatetime_PadZeroesIndex(ymdhmsu, 4, 2); // pad mm
    ymdhmsu = _enDatetime_PadZeroesIndex(ymdhmsu, 5, 2); // pad ss
    return
        ...
        + _enDatetime_DecimalizeSubseconds((integer)llList2String(ymdhmsu, 6), TRUE) // .ff..f
        + "Z";
}
...
)
{
    ...
    if (flags & FLAG_ENDATETIME_12_HOUR)
    {
        ...
        if (flags & FLAG_ENDATETIME_12_HOUR_M) xm += "M";
        if (flags & FLAG_ENDATETIME_12_HOUR_SPACE) xm = " " + xm;
...
@param integer flags FLAG_ENDATETIME_PAD_ZEROES_* flags. May include other flags.
@return list [h, m, s] with leading zero padding as requested.
*/
list _enDatetime_PadZeroesHMS(
    list hms,
    integer flags
)
{
    if (flags & FLAG_ENDATETIME_PAD_ZEROES_HOURS) hms = _enDatetime_PadZeroesIndex(hms, 0, 2);
    if (flags & FLAG_ENDATETIME_PAD_ZEROES_MINUTES) hms = _enDatetime_PadZeroesIndex(hms, 1, 2);
    if (flags & FLAG_ENDATETIME_PAD_ZEROES_SECONDS) hms = _enDatetime_PadZeroesIndex(hms, 2, 2);
    return hms;
}
...
*/
list _enDatetime_PadZeroesIndex(
    list l,
